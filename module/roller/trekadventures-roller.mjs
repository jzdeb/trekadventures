export class Roller2D20 {
    ...
    static async sendToChat({ rollname = "Roll xD20", roll = null, successTreshold = 0, critTreshold = 1, complicationTreshold = 20, dicesRolled = [], rerollIndexes = [], actorId = null, itemId = null } = {}) {
        ...
        const html = await renderTemplate("systems/trekadventures/templates/chat/roll2d20.html", rollData);
        let trekadventuresRoll = {}
        trekadventuresRoll.rollname = rollname;
        trekadventuresRoll.dicesRolled = dicesRolled;
        trekadventuresRoll.successTreshold = successTreshold;
        trekadventuresRoll.critTreshold = critTreshold;
        trekadventuresRoll.complicationTreshold = complicationTreshold;
        trekadventuresRoll.rerollIndexes = rerollIndexes;
        trekadventuresRoll.diceFace = "d20";
        ...
        let chatData = {
            ...
            content: html,
            flags: { trekadventuresroll: trekadventuresRoll },
    ...
    static async addD6({ rollname = "Roll D6", dicenum = 2, trekadventuresRoll = null, dicesRolled = [], itemId = null, actorId = null } = {}) {
        let formula = `${dicenum}ds`;
        let _roll = new Roll(formula);
        await _roll.evaluate({ async: true });
        let newRollName = `${trekadventuresRoll.rollname} [+ ${dicenum} DC]`;
        let oldDiceRolled = trekadventuresRoll.dicesRolled;
        await Roller2D20.parseD6Roll({
    ...
    static async sendD6ToChat({ rollname = "Roll D6", roll = null, dicesRolled = [], rerollIndexes = [], itemId = null, actorId = null } = {}) {
                ...
                if (item.type === 'spell') {
                    ...
                    for (let de in item.system.effect) {
                        if (item.system.effect[de].value) {
                            ...
                            let damageEffectLabel = game.i18n.localize(`trekadventures.WEAPONS.damageEffect.${de}`);
                            let efectLabel = `<span data-tooltip="${Handlebars.helpers.getTooltipFromConfigKey('trekadventures.WEAPONS.effects.'+de)}">${damageEffectLabel}${rank}</span>`;
                    ...
                    for (let qu in item.system.qualities) {
                        if (item.system.qualities[qu].value) {
                            let quLabel = game.i18n.localize(`trekadventures.WEAPONS.qualities.${qu}`);
        ...
        const html = await renderTemplate("systems/trekadventures/templates/chat/rollD6.html", rollData);
        let trekadventuresRoll = {}
        trekadventuresRoll.rollname = rollname;
        trekadventuresRoll.dicesRolled = dicesRolled;
        trekadventuresRoll.damage = damage;
        trekadventuresRoll.effects = effects;
        trekadventuresRoll.rerollIndexes = rerollIndexes;
        trekadventuresRoll.diceFace = "d6";

        let chatData = {
            user: game.user.id,
            rollMode: game.settings.get("core", "rollMode"),
            content: html,
            flags: { trekadventuresroll: trekadventuresRoll, itemId: itemId, actorId: actorId },
            type: CONST.CHAT_MESSAGE_TYPES.ROLL,
            roll: roll,
        };
        if (["gmroll", "blindroll"].includes(chatData.rollMode)) {
